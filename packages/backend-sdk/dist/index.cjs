"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/index.ts
var src_exports = {};
__export(src_exports, {
  parseDocument: () => parseDocument,
  serializePatches: () => serializePatches,
  validateMarkdown: () => validateMarkdown
});
module.exports = __toCommonJS(src_exports);
var import_core = require("@milkdown-agent/core");
function parseDocument(textWithPatches, format = "aider") {
  switch (format) {
    case "aider":
      return (0, import_core.parseAiderFormat)(textWithPatches);
    case "anthropic":
      throw new Error("Anthropic format not yet implemented");
    case "unified-diff":
      throw new Error("Unified diff format not yet implemented");
    default:
      throw new Error(`Unknown format: ${format}`);
  }
}
function serializePatches(patches, format = "aider") {
  switch (format) {
    case "aider":
      return (0, import_core.serializeAiderFormat)(patches);
    case "anthropic":
      throw new Error("Anthropic format not yet implemented");
    case "unified-diff":
      throw new Error("Unified diff format not yet implemented");
    default:
      throw new Error(`Unknown format: ${format}`);
  }
}
function validateMarkdown(document) {
  return (0, import_core.validateMarkdown)(document);
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  parseDocument,
  serializePatches,
  validateMarkdown
});
